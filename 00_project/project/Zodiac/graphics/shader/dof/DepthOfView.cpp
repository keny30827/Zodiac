#include "DepthOfView.h"
#include "../../GraphicsController.h"

void CDepthOfView::Init(CGraphicsController& graphicsController)
{
	...
	// シェーダービルド.
	{
		...
		// 頂点シェーダー.
		{
			ret = D3DCompileFromFile(
				L"shader\\dof\\DepthOfViewVS.hlsl",
				nullptr,
		...
		{
			ret = D3DCompileFromFile(
				L"shader\\dof\\DepthOfViewPS.hlsl",
				nullptr,
				D3D_COMPILE_STANDARD_FILE_INCLUDE,
				"main",
				"ps_5_0",
...
void CDepthOfView::Update()
{
}

void CDepthOfView::RenderSetup(CCommandWrapper& commandWrapper, CHeapWrapper& heapWrapper)
{
	// 情報が足らない.
	VRETURN(m_inputBaseRT);
	VRETURN(m_inputBlurMipTopRT);
	VRETURN(m_inputBlurMipOtherRT);
	...
	// コマンドリストへの設定.
	{
		...
		// 今回の描画におけるルートパラメータとの紐づけ.
		{
			...
			// ブラーのミップトップ.
			commandWrapper.SetGraphicsRootDescriptorTable(2, heapWrapper.GetGPUDescriptorHandle(HEAP_CATEGORY_HUGE, m_inputBlurMipTopRT->GetSrvHeapPosition()));
			// ブラーのミップ残り.
