...
void CHierarchicalZ::Init(CGraphicsController& graphicsController)
{
	...
	// コンスタントバッファーの準備.
	{
		...
		{
			// サイズ計算用の情報.
			const int dataSize = util::AdjustRowPitch(static_cast<uint32_t>(sizeof(SShaderSimpleSpriteInfo)), 256);
	...
	// シェーダービルド.
	{
		...
		// 頂点シェーダー.
		{
			ret = D3DCompileFromFile(
				L"shader\\hierarchical_z\\HierarchicalZVS.hlsl",
				nullptr,
		...
		{
			ret = D3DCompileFromFile(
				L"shader\\hierarchical_z\\HierarchicalZPS.hlsl",
				nullptr,
				D3D_COMPILE_STANDARD_FILE_INCLUDE,
				"main",
				"ps_5_0",
...
void CHierarchicalZ::Update()
{
}

void CHierarchicalZ::RenderSetup(CCommandWrapper& commandWrapper, CHeapWrapper& heapWrapper)
{
	// 情報が足らない.
