...
void CBloom::Init(CGraphicsController& graphicsController)
{
...
void CBloom::RenderSetup(CCommandWrapper& commandWrapper, CHeapWrapper& heapWrapper)
{
	// 情報が足らない.
	VRETURN(m_inputBaseRT);
	VRETURN(m_inputBloomMipTopRT);
	VRETURN(m_inputBloomMipOtherRT);
	...
	// コマンドリストへの設定.
	{
		...
		// 今回の描画におけるルートパラメータとの紐づけ.
		{
			...
			// 輝度値のミップトップ.
			commandWrapper.SetGraphicsRootDescriptorTable(2, heapWrapper.GetGPUDescriptorHandle(HEAP_CATEGORY_HUGE, m_inputBloomMipTopRT->GetSrvHeapPosition()));
			// 輝度値のミップ残り.
			commandWrapper.SetGraphicsRootDescriptorTable(3, heapWrapper.GetGPUDescriptorHandle(HEAP_CATEGORY_HUGE, m_inputBloomMipOtherRT->GetSrvHeapPosition()));
		}
	}
}