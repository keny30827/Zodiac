...
void CSSAO::Init(CGraphicsController& graphicsController)
{
...
void CSSAO::RenderSetup(CCommandWrapper& commandWrapper, CHeapWrapper& heapWrapper)
{
	// 情報が足らない.
	...
	// コマンドリストへの設定.
	{
		...
		// 今回の描画におけるルートパラメータとの紐づけ.
		{
			...
			// 法線.
			commandWrapper.SetGraphicsRootDescriptorTable(1, heapWrapper.GetGPUDescriptorHandle(HEAP_CATEGORY_HUGE, m_inputNormalRT->GetSrvHeapPosition()));
			// 深度.
			commandWrapper.SetGraphicsRootDescriptorTable(2, heapWrapper.GetGPUDescriptorHandle(HEAP_CATEGORY_HUGE, m_inputDepthRT->GetSrvHeapPosition()));
		}
	}
}